...
from yolov6.models.losses.loss_distill_R_compare import ComputeLoss as ComputeLoss_distill_compare
...
from yolov6.distiller.build_distiller import build_distiller
...
class Trainer:
    def __init__(self, args, cfg, device):
        ...
        if self.args.distill:
            ...
            if self.args.distill_compare:
                print(cfg.distiller)
                self.distiller = build_distiller(cfg.distiller)
                LOGGER.info("The comparsion is be intialized"+ self.distiller.name)
        ...
        elif self.args.distill_compare:
            self.optimizer = self.get_optimizer(args, cfg, model, self.distiller)
        else:
        ...
        if self.distill_ns:
            ...
            self.progress = Progress(
                ...
                TextColumn("[red]{task.fields[cwd_loss]}"),
                "•",
                TextColumn("[red]{task.fields[ass_loss]}"),
                BarColumn(
                    bar_width=None,
                    style="white",
                    complete_style="blue",
    ...
    def train_before_loop(self):
        ...
        if self.args.distill:
            ...
            if self.distill_ns:
                ...
                distill_assigner=self.args.distill_assigner,
                loss_weight=self.cfg.loss.loss_weight,
                max_epoch = self.max_epoch
            )
            elif self.args.distill_compare:
                Loss_distill_func = ComputeLoss_distill_compare
                LOGGER.info("The current distill method mode is Compare method")
                self.compute_loss_distill = Loss_distill_func(
                num_classes=self.data_dict["nc"],
                ori_img_size=self.img_size,
                fpn_strides=self.cfg.model.head.strides,
        ...
        if self.main_process:
            ...
            if not self.args.distill:
                ...
                self.task = self.progress.add_task(
                    ...
                    ass_loss=f"ass{self.mean_loss[5]:7.4g}",
                )  
            self.progress.start()
    ...
    def get_model(self, args, cfg, nc, device, distill_ns):
        model = build_model(
            cfg, nc, device, fuse_ab=self.args.fuse_ab, distill_ns=distill_ns
        )
    ...
    def get_optimizer(self, args, cfg, model, distiller = None):
        ...
        optimizer = build_optimizer(cfg, model,distiller=distiller)
        return optimizer
