...
class ComputeLoss:
    ...
    distill_off=False):
        ...
        loss_angle, d_loss_angle = self.angle_loss(
            pred_angles, t_pred_angles, target_angles, target_scores, target_scores_sum, fg_mask, 10
        )
        # d_loss_angle = self.distill_loss_cls(pred_angles, t_pred_angles,180,temperature=20)
        ...
        # d_loss_cls = self.distill_loss_cls_foreground(logits_student, logits_teacher, distill_num_classes, 10, fg_mask)
        # NOTE DIOR-R test
        d_loss_cls = self.distill_loss_cls(logits_student, logits_teacher, distill_num_classes, 10)
        if target_scores_sum > 0:
        ...
        loss = (
                ...
                + self.loss_weight['ass'] * d_loss_assigner
            )
        return (
                loss,
                torch.cat(
                    (
                        ...
                        (self.loss_weight['ass'] * d_loss_assigner*3).unsqueeze(0),
                    )
                ).detach(),
            )
    ...
    def distill_loss_assigner(self, align_metric_t, align_metric_s, mask_pos_in_gt, mask_gt, temperature=20):
        ...
        align_student = F.log_softmax(align_metric_s/temperature, dim=2)
        # print(torch.min(align_student[0,0,...]))
        # align_student = torch.where(mask_pos_in_gt>0, align_student, torch.full_like(align_student,0.0))
        align_teacher = F.log_softmax(align_metric_t/temperature, dim=2)
        # align_teacher = torch.where(mask_pos_in_gt>0, align_teacher, torch.full_like(align_teacher,1.0))
        # print(torch.topk(align_metric_s[0,0,...],10))
        gt_num = torch.sum(mask_gt>0)
        mask_pos_in_gt = torch.sum(mask_pos_in_gt>0)
        # print(mask_gt.shape)
        mask_mask_gt = mask_gt.repeat(1,1,num_anchors)
        # print(mask_mask_gt.shape)
        student_logit = torch.masked_select(align_student, mask_mask_gt.bool()).reshape(gt_num,-1)
        teacher_logit = torch.masked_select(align_teacher, mask_mask_gt.bool()).reshape(gt_num,-1)
        
        mask_ass_gt = torch.masked_select(mask_pos_in_gt,mask_mask_gt).reshape(gt_num,-1)
        
        
        
        demo = F.kl_div(student_logit,teacher_logit.detach(),reduce="sum",log_target=True)*temperature*temperature*num_anchors
        
        # demo = F.mse_loss(student_logit, teacher_logit, reduction="sum") * 50 * 100
        # NOTE GTR 添加到100
        # print(demo)
...
class AngleLoss(nn.Module):
    ...
    def distill_loss_angle(self, logits_student, logits_teacher, csl_label,temperature):
        logits_student = logits_student.view(-1, self.angle_max)
        logits_teacher = logits_teacher.view(-1, self.angle_max)
        pred_student = F.softmax(F.sigmoid(logits_student.clone()) / temperature, dim=1)
        pred_teacher = F.softmax(F.sigmoid(logits_teacher.clone()) / temperature, dim=1)
        log_pred_student = torch.log(pred_student)
        d_loss_angle = F.kl_div(log_pred_student, pred_teacher, reduction="sum")
    ...
    def forward(
        ...
        if num_pos > 0:
            ...
            if self.angle_fitting_methods == "regression":
                ...
                d_loss_angle = self.distill_loss_angle(pred_angle_pos, t_pred_angle_pos, csl_gaussian_target_angle_pos,temperature,) * angle_weight
            # dfl loss
            elif self.angle_fitting_methods == "dfl":
