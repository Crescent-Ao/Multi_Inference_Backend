...
class TaskAlignedAssigner(nn.Module):
    def __init__(self, topk=13, num_classes=80, alpha=1.0, beta=5.0, eps=1e-9, max_epoch=100):
    ...
    def forward(self, pd_scores, pd_bboxes, pd_angles, anc_points, gt_labels, gt_bboxes, gt_angles, mask_gt, epoch_num):
        ...
        """
        topk_decay = ((1 - torch.cos(epoch_num * torch.pi / torch.tensor(self.max_epoch))) / 2) * (
            0.01 - 1)+1
        self.topk = int(torch.round(self.min_topk*(1+topk_decay)))
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, pd_angles, gt_labels, gt_bboxes, gt_angles):
        ...
        align_metric = bbox_scores.pow(self.alpha) * (overlaps*angle_ratio).pow(self.beta) 
